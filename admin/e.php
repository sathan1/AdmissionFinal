...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['export'])) {
    ...
    if ($format === 'pdf') {
        ...
        $columns = [
            'S.No', 'Department', 'Type', 'Shift', 'Hindu (B)', 'Hindu (G)', 'Muslim (B)', 'Muslim (G)', 
            'Christian (B)', 'Christian (G)', 'Jain (B)', 'Jain (G)', 'Sikh (B)', 'Sikh (G)', 
            'Buddhist (B)', 'Buddhist (G)', 'Others (B)', 'Others (G)', 'Total (B)', 'Total (G)', 
            'Overall Total'
        ];
        ...
        $sheet->getStyle('A' . $rowNum . ':U' . $rowNum)->applyFromArray([
            'fill' => [
                'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
                'startColor' => ['rgb' => 'CCE5FF'], // Light blue to match PDF
            ],
            'font' => [
                'bold' => true,
            ],
...
<head>
    ...
    <style>
        ...
        .table-success td {
            background-color: #cce5ff; /* Changed from green to match export styling */
            font-weight: bold;
        }
